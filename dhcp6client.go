...
// was: Package nclient6 is a minimum-functionality client for DHCPv6.
// renamed to dhcp6c: this is an extended version with more features
// added/changed:
// logger is new public Logger
// added WithLogger(logger Logger)
package dhcp6c
...
func (e emptyLogger) Printf(format string, v ...any)                      {}
...
func (s shortSummaryLogger) Printf(format string, v ...any) {
...
func (c *Client) PrintMessage(prefix string, message *dhcpv6.Message) {
	c.logger.PrintMessage(prefix, message)
}

// iapd

// WithIAPD adds an IAPD option with the provided IAID and
// prefix option to a DHCPv6 packet.
// no check is done if same iaid is added again
func WithIAPD(iaid [4]byte, prefix *dhcpv6.OptIAPrefix) dhcpv6.Modifier {
	return func(d dhcpv6.DHCPv6) {
		if _, ok := d.(*dhcpv6.Message); ok {
			var opt = &dhcpv6.OptIAPD{}
			copy(opt.IaId[:], iaid[:])
